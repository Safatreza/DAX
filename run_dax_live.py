"""
RSL Live Run — DAX 40: fetches real-time data and produces rankings.
Same Relative Strength Levy strategy as the S&P 500 / Nasdaq-100 versions.
"""
...
def hole_dax_ticker():
    ...
    print("\n[1/4] Lade DAX 40 Ticker von Wikipedia...")
    try:
        ...
        if table is None:
            for t in soup.find_all('table', {'class': 'wikitable'}):
                ...
                if headers_row and any('ticker' in th.get_text().lower() or 'symbol' in th.get_text().lower()
        ...
        for c in df.columns:
            ...
            if 'ticker' in cl or 'symbol' in cl:
                col_map[c] = 'Symbol'
            elif 'company' in cl or 'name' in cl or 'unternehmen' in cl:
                col_map[c] = 'Unternehmen'
            elif 'sector' in cl or 'sektor' in cl or 'gics' in cl:
                col_map[c] = 'Sektor'
            elif 'industry' in cl or 'branche' in cl:
                col_map[c] = 'Branche'
        df = df.rename(columns=col_map)

        # DAX tickers need .DE suffix for yfinance; Wikipedia may or may not include it
        def normalise(t):
            t = str(t).strip()
            if t.upper().endswith('.DE'):
                return t  # already correct
            return t + '.DE'

        result = pd.DataFrame({
            'Symbol':      df['Symbol'].apply(normalise),
        ...
        fallback = [
            ...
            ('MRK.DE','Merck'),('DTE.DE','Deutsche Telekom'),('BAYN.DE','Bayer'),
            ('BMW.DE','BMW'),('MBG.DE','Mercedes-Benz'),('VOW3.DE','Volkswagen'),
            ('ADS.DE','Adidas'),
        ]
        return pd.DataFrame({
            'Symbol':      [t for t,_ in fallback],
            'Unternehmen': [n for _,n in fallback],
            'Sektor':      ['Diverse']*len(fallback),
            'Branche':     ['Diverse']*len(fallback),
        })

def hole_dax_kurse(start, end):
    try:
        h = yf.Ticker('^GDAXI').history(start=start, end=end, auto_adjust=True)
        return h['Close'] if not h.empty else None
    except Exception:
        return None

# ── Per-stock data ─────────────────────────────────────────────────────────────
def hole_aktien_daten(ticker, start, end, dax_kurse=None, max_versuche=3):
    hist = None
    tk   = yf.Ticker(ticker)
    ...
    try:
        ...
        rel_dax = None
        if dax_kurse is not None and len(dax_kurse) >= RSL_PERIODE:
            ap = berechne_aenderung(closes, RSL_PERIODE)
            dp = berechne_aenderung(dax_kurse, RSL_PERIODE)
            if ap is not None and dp is not None:
                rel_dax = round(ap - dp, 2)
...
# ── Batch processing ───────────────────────────────────────────────────────────
def verarbeite_alle(ticker_df, start, end, dax_kurse):
    ergebnisse     = []
    fehlgeschlagen = []
    ...
    for _, zeile in tqdm(ticker_df.iterrows(), total=len(ticker_df), desc="Lade Daten"):
        daten = hole_aktien_daten(zeile['Symbol'], start, end, dax_kurse)
    ...
    if not ergebnisse:
        print("  FEHLER: Keine Daten erhalten — alle Ticker fehlgeschlagen.")
...
def erstelle_excel(df, top, sektor_stats, fehler, datei):
    ...
    with pd.ExcelWriter(datei, engine='xlsxwriter') as writer:
        ...
        ttl = wb.add_format({'bold': True, 'font_size': 14, 'font_color': '#1F3864'})
        ...
        cols1 = ['Rang','Ticker','Unternehmen','Sektor','Branche','Aktueller_Kurs','MktCap_Text',
                 ...
                 'Proz_vom_Hoch','Proz_vom_Tief','Volumen_Ratio','Beta','KGV','Dividendenrendite',
                 'ATR_Prozent','ATR_Warnung_Text']
        hdrs1 = ['Rang','Ticker','Unternehmen','Sektor','Branche','Kurs (€)','Marktkapitalisierung',
                 ...
                 '% vom Hoch','% vom Tief','Vol. Ratio','Beta','KGV','Div. Rendite (%)',
                 'ATR % (Tag)','Warnsignal']
        ...
        ws1.set_column('A:A', 6); ws1.set_column('B:B', 10); ws1.set_column('C:C', 26)
        ws1.set_column('D:E', 22); ws1.set_column('F:Y', 12)
        ...
        hdrs2 = ['Rang','Ticker','Unternehmen','Sektor','Kurs (€)','Marktkapitalisierung',
                 'RSL 26T','Perzentil (%)','Rel. Stärke vs DAX',
        ...
        ws2.set_column('A:A', 6); ws2.set_column('B:B', 10); ws2.set_column('C:C', 26)
        ...
        ws3.set_column('A:A', 28); ws3.set_column('B:H', 16); ws3.freeze_panes(1, 0)

        ws4 = wb.add_worksheet('Zusammenfassung')
        ws4.write(0, 0, 'RSL DAX 40 SCREENING — ZUSAMMENFASSUNG', ttl)
        ...
        lines = [
            ('RSL DAX 40 SCREENING — METHODIK', True),
            ('', False),
            ...
            ('', False),
            ('INDEX', True),
            ('DAX 40 — die 40 größten deutschen Unternehmen (Frankfurter Börse / XETRA)', False),
            ('', False),
            ('DATEN', True),
            ('Ticker:  Wikipedia — DAX (mit .DE-Suffix für Yahoo Finance)', False),
            ('Kurse:   Yahoo Finance (yfinance)', False),
            ('Index:   DAX (^GDAXI)', False),
            ('Währung: Euro (€)', False),
            ('', False),
...
# ── Main ───────────────────────────────────────────────────────────────────────
dax_df = hole_dax_ticker()

end_datum   = datetime.now()
start_datum = end_datum - timedelta(days=RUECKBLICK_TAGE)

print("\n[2/4] Lade DAX-Indexdaten...")
dax_kurse = hole_dax_kurse(start_datum, end_datum)
print(f"  DAX: {len(dax_kurse) if dax_kurse is not None else 0} Datenpunkte geladen.")

ergebnis_df, fehlgeschlagene = verarbeite_alle(dax_df, start_datum, end_datum, dax_kurse)
...
for _, r in ergebnis_df.head(10).iterrows():
    print(f"  {r['Rang']:>3}. {r['Ticker']:<12}  RSL={r['RSL']}  26T={r['Aenderung_26T']:+.1f}%")
...
def erstelle_json(df, sektor_stats, fehler):
        ...
        "rankings": [
            {
                ...
                "rel_vs_dax":     safe(r['Rel_Staerke_DAX']),
                "beta":           safe(r['Beta']),
                "pe_ratio":       safe(r['KGV']),
    ...
    json_path = os.path.join('web', 'data', 'dax_rankings.json')
